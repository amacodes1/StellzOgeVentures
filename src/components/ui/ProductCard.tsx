...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return <div className="bg-white rounded-lg shadow-md overflow-hidden flex flex-col relative">
      {showWishlist && (
        <button
          type="button"
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <div className="mt-auto">
          <div className="flex justify-between items-center mb-3">
            ...
            <span className={`text-xs px-2 py-1 rounded-full font-medium ${inStock ? 'bg-secondary-100 text-secondary-700' : 'bg-red-100 text-red-700'}`}>
              {inStock ? 'In Stock' : 'Out of Stock'}
            </span>
          </div>
          {minOrderQuantity > 1 && <p className="text-xs text-gray-500 mb-3">
              Min. Order: {minOrderQuantity} units
            </p>}
          <div className="flex space-x-2">
            <Button 
              className="bg-primary-600 hover:bg-primary-700 text-white flex-1 flex items-center justify-center" 
